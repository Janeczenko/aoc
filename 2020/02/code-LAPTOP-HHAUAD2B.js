...
3-4 x: gxxjphxx
4-6 g: sbwggg`;
var table=passwords.split(`
`);

var correct=0;
for(let i=0;i<table.length;i++)
{
	let chars_min=parseInt(table[i].substr(0,table[i].indexOf("-")));
	let chars_max=parseInt(table[i].substr(table[i].indexOf("-")+1,table[i].indexOf([/a-z/])-table[i].indexOf("-")-2));
	let chars_current=0;
	let letter=table[i].substr(table[i].indexOf([/a-z/]));
	let password=table[i].substr(table[i].indexOf(":")+2,table[i].length-table[i].indexOf(";")-2);
	for(let j=0;j<password.length;j++)
	{
		if(password[j]==letter)chars_current++;
	}
	if(chars_current>=chars_min && chars_current<=chars_max)correct++;
}
