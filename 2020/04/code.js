...
var cat=["byr","iyr","eyr","hgt","hcl","ecl","pid"];
var correct=0;
for(let i=0;i<table.length;i++)
{
	table[i]=table[i].replaceAll("\n"," ");
	table[i]=table[i].split(" ");
	let ver=[false,false,false,false,false,false,false];
	for(let j=0;j<table[i].length;j++)
	{
		for(let k=0;k<cat.length;k++)
		{
			if(table[i][j].substr(0,3)==cat[k])
			{
				ver[k]=true;
			}
		}
	}
	if(ver.indexOf(false)==-1)
	{
		correct++;
	}
}
...
for(let i=0;i<table.length;i++)
{
	...
	for(let j=0;j<table[i].length;j++)
	{
		if(table[i][j].substr(0,3)=="byr"&&parseInt(table[i][j].substr(-4))>=1920&&parseInt(table[i][j].substr(-4))<=2002)ver[0]=true;
		if(table[i][j].substr(0,3)=="iyr"&&parseInt(table[i][j].substr(-4))>=2010&&parseInt(table[i][j].substr(-4))<=2020)ver[1]=true;
		if(table[i][j].substr(0,3)=="eyr"&&parseInt(table[i][j].substr(-4))>=2020&&parseInt(table[i][j].substr(-4))<=2030)ver[2]=true;
		if(table[i][j].substr(0,3)=="hgt"&&((table[i][j].substr(table[i][j].length-2)=="cm"&&parseInt(table[i][j].substr(table[i][j].indexOf(":")+1,3))>=150&&parseInt(table[i][j].substr(table[i][j].indexOf(":")+1,3))<=193)||(table[i][j].substr(table[i][j].length-2)=="in"&&parseInt(table[i][j].substr(table[i][j].indexOf(":")+1,2))>=59&&parseInt(table[i][j].substr(table[i][j].indexOf(":")+1,2))<=76)))ver[3]=true;
		if(table[i][j].substr(0,3)=="hcl"&&/#[0-9a-f]{6}/.test(table[i][j].substr(-7)))ver[4]=true;
		if(table[i][j].substr(0,3)=="ecl"&&/amb|blu|brn|gry|grn|hzl|oth/.test(table[i][j].substr(-3)))ver[5]=true;
		if(table[i][j].substr(0,3)=="pid"&&table[i][j].substr(table[i][j].indexOf(":")+1).length==9&&/[0-9]{9}/.test(table[i][j].substr(-9)))ver[6]=true;
	}
	if(ver.indexOf(false)==-1)
	{
		correct++;
