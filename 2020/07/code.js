var input=`mirrored gold bags contain 3 light teal bags.
...
{
	if(/other/.test(data[i]))
	{
		data.splice(i,1);
		continue;
	}
	data[i]=Array.from(data[i].matchAll(/(?:\d\s)?\w+\s\w+\sbag/g), m => m[0]);
	for(let j=0;j<data[i].length;j++)
	{
		data[i][j]=data[i][j].replace(/\sbag/,"");
		if(j>0)
		{
			data[i][j]=data[i][j].replace(" ","#");
			data[i][j]=data[i][j].split("#");
			data[i][j][0]=parseInt(data[i][j][0]);
		}
	}
}
/*
...
bags.sort();
document.write(bags.length);
*/
var bags=[[1,"shiny gold"]];
var recur=true;
var bagsAmt=-1;
while(recur)
{
	...
	for(let i=bags.length-1;i>=0;i--)
	{
		...
		{
			if(data[j][0]==bags[i][1])
			{
				for(let k=1;k<data[j].length;k++)
				{
					bags.push([data[j][k][0]*bags[i][0],data[j][k][1]]);
				}
				bagsAmt+=bags[i][0];
				bags.splice(i,1);
				recur=true;
